"""Direct-npz download for the ROM sweep, mirroring Basic_FE_sweeps'
npz_direct_pipeline/download_npz_results.py.

Pulls `config.json` plus every per-simulation `npz/sim_*.npz` file straight
from the cluster -- no combined results.pkl/collector step. Downloading is
network/round-trip-latency bound, not CPU bound, so this launches N_WORKERS
concurrent rsync streams (via WSL), each restricted to sim indices whose
last digit falls in its own bucket, so transfers overlap instead of queuing
behind one another. config.json is fetched once, separately, since it's tiny
and unrelated to the per-sim sharding.
"""
import subprocess
import shlex
...
dirnaeme = "ROM_Duffing_freq_sweep_REPLACE_WITH_JOB_ID"
REMOTE_DIR = (
	f"/storage/home/hcoda1/4/setemadi3/scratch/Modeling/tasks/Basic_ROM_sweeps/sim_dat/{dirnaeme}/"
)

DEST_WSL = (
	"/mnt/e/GaTech Dropbox/Seyednima Etemadi/"
	"Projects/Metamaterial beam/"
	f"Modeling/tasks/Basic_ROM_sweeps/sim_dat/{dirnaeme}"
)
