% load ..\..\Parameters_tunable.mat Tspan M_boundary V_boundary Lj Lcj
% Lj_old = Lj;
% Lcj_old = Lcj;
% clear Lcj Lj
% clear Lcj
...
Lj = 9.2237;
load disp_BC_new.mat Tspan w0_boundary d_dt_w0_boundary d2_dt2_w0_boundary
% Amp = 0.2;
...
%% Boundary condition
% The current BC is pin-pin
% bcdof = 2 * N_ele_seg * N_segs + [1 2];
bcdof = [1 2 * N_ele_seg * N_segs + 1];
...
index_dof(:,bcdof) = [];
index_deflection_dof = index_dof(2,:)==1;
index_slope_dof = index_dof(2,:)==2;
index_circuits_dof = index_dof(2,:)==3;
index_dof_total = 1:N_dof_total;
index_dof_free = index_dof(1,:)';
index_boundary = bcdof';
% N_dof_eff = N_dof_total - size(bcdof,2);
% index_dof_total = 1:N_dof_eff;
% index_boundary = [1 2 N_dof_eff];
% N_dof_free = N_dof_eff - size(index_boundary,2);
stiffness_M_free = stiffness_M(index_dof_free,index_dof_free);
mass_M_free = mass_M(index_dof_free,index_dof_free);
damping_eff_M_free = damping_eff_M(index_dof_free,index_dof_free);
stiffness_M_boundary = stiffness_M(index_dof_free,index_boundary);
mass_M_boundary = mass_M(index_dof_free,index_boundary);
damping_eff_M_boundary = damping_eff_M(index_dof_free,index_boundary);
%% Boundary excitation
% Displacement prescription

X_boundary = [w0_boundary(1);0];
dot_X_boundary = [d_dt_w0_boundary(1);0];
ddot_X_boundary = [d2_dt2_w0_boundary(1);0];
f_ext_full_0 = - mass_M_boundary * ddot_X_boundary - damping_eff_M_boundary * dot_X_boundary - stiffness_M_boundary * X_boundary;

%% Initial condition
w0_IC = w0_boundary(1);
dw = (0 - w0_IC)/(N_ele_seg * N_segs);

X0_deflection = w0_IC:dw:0;
X0_slope = dw/dx * ones(N_ele_seg * N_segs + 1,1);
X0_deflection(1) = []; % BCdof
X0_deflection(end) = []; % BCdof

% only applicable to pin-pin boundary condition
X0 = zeros(N_dof_eff, 1);
X0(index_deflection_dof) = X0_deflection;
X0(index_slope_dof) = X0_slope;
% X0(index_circuits_dof) = 
...
a0 = mass_M_free \ ( - damping_eff_M_free * v0 - stiffness_M_free * X0 - f_nl_full_0 + f_ext_full_0);
...
for i_n = 1:size(Tspan,2)-1
    ...
    Xn1_temp = Xn;
    % External force
    wb_temp = w0_boundary(i_n + 1);
    dot_wb_temp = d_dt_w0_boundary(i_n + 1);
    ddot_wb_temp = d2_dt2_w0_boundary(i_n + 1);
    X_boundary = [wb_temp;0];
    dot_X_boundary = [dot_wb_temp;0];
    ddot_X_boundary = [ddot_wb_temp;0];
...
figure;
filename = 'moving_soliton_deflection_new_new.gif';

Deflection_t_frame = [0;XT(index_deflection_dof,1);0];
hold on
h1 = plot(x_vec_beam,Deflection_t_frame);
ylim([-2e-5, 2e-5]);  % Set y-axis limits for better visualization
xlabel('x');
...
nFrames = 100 + 1;
Frame_dtstep = (size(Tspan,2)-1)/(nFrames - 1);
pause(0.15);
for i_frame = 2:nFrames
    i_frame
    % Update the y-values (shifting the sine wave)
    i_tstep_frame = (1 + (i_frame - 1) * Frame_dtstep);
    t_frame = Tspan(i_tstep_frame);
    Deflection_t_frame = [0;XT(index_deflection_dof,i_tstep_frame);0];
    % Update the plot data
    set(h1, 'YData', Deflection_t_frame);
    title(['T =',num2str(t_frame)]);
    frame = getframe(gcf);
    img = frame2im(frame);
    [imind, cm] = rgb2ind(img, 256);
...
%%
plot(x_vec_beam,[0;XT(index_deflection_dof,10000);0])
% plot(1:300,[XT(index_circuits_dof,15000)])
% plot(x_vec_beam(2:end),[aT_sample(index_slope_dof,201)])
% plot(1:300,[XT_sample(index_circuits_dof,201)])
...
save soft_cantilever_Boundary_optical_new.mat Tspan_sample XT_sample VT_sample aT_sample index_deflection_dof index_slope_dof index_circuits_dof Lj Lcj