...
K_i = 0.001; K_p = 0.03; K_c = 5e11
# param_list = np.array([0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.4]) * 125 # Amplitude sweep
param_list = np.arange(0.01, 0.2, 0.01) # K_p sweep
t_end = 1
f0 = 1300
f1 = 3000
...
def run_single_simulation(par, K_c, K_p, K_i, t_end, f0, f1, x_eval, t_eval):
	...
	res = run_time_sim(v_exc=v_exc, K_c=K_c, K_p=par, K_i=K_i, t_end=t_end, x_eval=x_eval, t_eval=t_eval)
...
sim_results = Parallel(n_jobs=20, verbose=10)(
...
#%%
np.savez('./'+f"sim_dat/NES_kp_sweep_Kc={K_c:.2e}"+'.npz', **results)
