...
results = run_time_sim(v_exc=v_exc, K_c=K_c, K_p=K_p, K_i=K_i, t_end=t_end, x_eval=x_eval, t_eval=t_eval)
t = results['t']
veloc = results['veloc']
freq = results['freq']
Y = results['Y']
...
plt.semilogy(comsol_SC['freq'], comsol_SC['w']*2*pi*comsol_SC['freq'], 'g-', label='COMSOL ')
...
plt.semilogy(frq_SC_exp, np.mean(frf_data_SC_exp[:,:], axis=1), 'k--', label=f'Experiment')
