...
[V, D] = eigs(stiffness_M_augmented,mass_M_augmented,6, 'smallestabs','Tolerance',1e-15);
V_Deflection_raw = [0;real(V(1:2:79,1))];
V_Slope_raw = [0;real(V(2:2:80,1))];
x_beam_vec_FEM = 0:l/40:l;
% plot(x_beam_vec_FEM,V_Deflection_raw)
Mod2 = 0;
for i_ele = 1:N_ele
    deflection_raw_1 = V_Deflection_raw(i_ele);
    deflection_raw_2 = V_Deflection_raw(i_ele+1);
    Slope_raw_1 = V_Slope_raw(i_ele);
    Slope_raw_2 = V_Slope_raw(i_ele+1);
...
int_0_l_phi_r_FEM = 0;
for i_ele = 1:N_ele
    deflection_1 = V_Deflection(i_ele);
    deflection_2 = V_Deflection(i_ele+1);
    Slope_1 = V_Slope(i_ele);
    Slope_2 = V_Slope(i_ele+1);
    int_phi_ele = int_phi(0,l_ele,deflection_1,deflection_2,Slope_1,Slope_2);
    int_0_l_phi_r_FEM = int_0_l_phi_r_FEM + int_phi_ele;
end
...
figure(1)
hold on
h11 = plot(x_beam_vec_FEM,V_Deflection,'LineWidth',1.5);
