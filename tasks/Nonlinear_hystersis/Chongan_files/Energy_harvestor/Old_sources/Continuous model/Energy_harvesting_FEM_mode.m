...
omega_v = 510; % Frequency of modulator
...
%%
figure(5)
F_enve = sqrt(((2 * Fb + 2 * Fv * sin((Omega_f2 - Omega_f) * T)) .^2 + (2 * Fv * cos((Omega_f2 - Omega_f) * T)).^2));
hold on
h51 = plot(abs(Fb_sol),abs(varphi_Lambda_1_vec),'k--',LineWidth=1.5);
h52 = scatter(F_enve/2, abs(varphi_Lambda_1_num),'.');
legend(['Analytical Hysteresis'],'Numerical tracking','Location','northwest')
set(gca,'fontsize',12)
xlabel(['$|F_b|$'], 'Interpreter', 'latex')
ylabel('${|\varphi_{\Lambda1}|}$', 'Interpreter', 'latex');
set(gca,'fontsize',12)
title(['Modulation frequency = ',num2str(omega_v-omega_b),'rad/s'])
%%
% Multiple Fbs
% Initial conditions
% x0 = [0; 0; 0; 0]; % [H1, dH1/dT, Lambda1, dLambda1/dT]
% Fb_vec = 0:-0.0001:-0.020;
% varphi_Lambda_1_num_steady = zeros(1,size(Fb_vec,2));
% % Time span
% Tspan = [0 3000];
% for i_Fb = 1:size(Fb_vec,2)
% % ODE function
% Fb = Fb_vec(i_Fb);
% odefun = @(T, x) [
%     x(2); % dx1/dT = x2
...
% varphi_Lambda_1_num_steady(i_Fb) = abs(X(end, 4) + 1j * Omega_f * X(end, 3));
% end
% figure(31)
% hold on
% h311 = plot(-Fb_vec,varphi_Lambda_1_num_steady,LineWidth=1);
% h312 = plot(abs(Fb_sol),abs(varphi_Lambda_1),'k--',LineWidth=1.5);
% legend([h311 h312],'Numerical Results','Analytical Results')
% xlabel(['$|F_b|$'], 'Interpreter', 'latex')
% ylabel('${|\varphi_{\Lambda1}|}$', 'Interpreter', 'latex');
% title(['Excite from trivial Initial condition'])
% set(gca,'fontsize',12)